...
export function SavingsForm({ selectedMonth, selectedYear }: SavingsFormProps) {
  ...
  const fetchSavings = async () => {
    const { data, error } = await supabase
      ...
      .single(); // Since there's only one savings goal per month
  ...
  const handleSubmit = async () => {
    ...
    // Use 'upsert' to either update the existing month or create a new one
    const { error } = await supabase.from("savings").upsert(
      ...
      { onConflict: "user_id,month,year" } // Ensures only one record per user/month/year
    );

    if (error) {
      console.error("Error saving savings:", error.message);
      alert("Failed to sync savings.");
    } else {
      setCurrentSavings(value);
      alert("Savings synced! 🐷✨");
    }
    setLoading(false);
  };
